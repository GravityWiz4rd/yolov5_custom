...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if opt.teacher_weight:
        ...
        teacher_model = Model(cfg or teacher_ckpt['model'].yaml, ch=3, nc=nc,
    ...
    if opt.teacher_weight:
        ...
        teacher_model.class_weights = labels_to_class_weights(
    ...
    if opt.teacher_weight:
        ...
        stu_feature_adapt = nn.Sequential(nn.Conv2d(student_channel, teacher_channel, 3, padding=1, stride=int(
            student_out_size / teacher_out_size)), nn.ReLU()).to(device)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                if opt.teacher_weight:
                    ...
                    loss, loss_items = compute_loss(pred, targets, teacher_feature.detach(
                    ), stu_feature_adapt(features), mask.detach())  # loss scaled by batch_size
                else:
